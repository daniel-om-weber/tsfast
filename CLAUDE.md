...
- Comments and docstrings describe the current state only, and only what the code can't say itself (constraints, invariants, non-obvious rationale). Never restate the adjacent code (`# Validate` above `validate()`).
- No history in the source. Why a value changed, what a past bug was, which commit changed it, what an earlier implementation did — that belongs in the commit message, where `git log` and `git blame` find it. Never write: "previously", "now uses", "no longer", "back to X now that Y", "Phase 2", "deferred", a commit SHA, or a quoted commit message. Rationale that still constrains the code is welcome, phrased as the constraint that holds today ("capture must be thread_local: a pinning DataLoader thread invalidates a global-mode capture") rather than as the story of how it got here. Exception: back-compat code may name the old format it exists to support.
