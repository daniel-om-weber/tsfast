...
__all__ = ['obj_in_lst', 'get_hdf_files', 'hdf_extensions', 'apply_df_tfms', 'CreateDict', 'DfHDFCreateWindows',
           'calc_shift_offsets', 'running_mean', 'downsample_mean', 'hdf_extract_sequence', 'Memoize', 'HDF2Sequence',
           ...
           'SeqNoiseInjection', 'encodes', 'decodes', 'ParentSplitter', 'PercentageSplitter', 'ApplyToDict',
           'pad_sequence', 'SequenceBlock', 'Seq2SeqDS', 'Seq2SeqDataloaders', 'plot_sequence',
           'plot_seqs_single_figure', 'plot_seqs_multi_figures']
...
#Cell
def hdf_extract_sequence(hdf_path,clms,dataset = None, l_slc = None, r_slc= None,down_s=None):
    with h5py.File(hdf_path,'r') as f:
        ds = f if dataset is None else f[dataset]
        l_array = [ds[n][l_slc:r_slc] for n in clms]
        seq = np.vstack(l_array).T
        if down_s is not None: seq = downsample_mean(seq,down_s)
        return seq
...
class HDF2Sequence(Transform):
    ...
    def __init__(self, clm_names,clm_shift=None,truncate_sz=None,down_s=None,to_cls=noop,cached=True):
        ...
        store_attr(self,'clm_names,clm_shift,truncate_sz,to_cls,cached,down_s')
    ...
    def _extract_dict_sequence(self,item):
        if isinstance(item,dict):
            ...
            if self.cached:
                seq = self._exseq(path,dataset,None,None,self.down_s)[l_slc:r_slc]
            else:
                seq = self._exseq(path,dataset,l_slc,r_slc,self.down_s)
        else:
            seq = self._exseq(str(item),None,None,None,self.down_s)
...
#Cell
def plot_sequence(axs,in_sig,targ_sig,out_sig=None):
    n_targ = targ_sig.shape[1]
    for j,ax in  enumerate(axs[:-1]):
        ax.plot(targ_sig[:,j])
        if out_sig is not None: ax.plot(out_sig[:,j])
        ax.label_outer()
    axs[-1].plot(in_sig)

#Cell
def plot_seqs_single_figure(n_samples,n_targ,samples,plot_func,outs=None):
...
#     import pdb; pdb.set_trace()
    for i in range(n_samples):
        ...
        plot_func(axs,in_sig,targ_sig,out_sig=out_sig if outs is not None else None)
    plt.tight_layout()

#Cell
def plot_seqs_multi_figures(n_samples,n_targ,samples,plot_func,outs=None):
    for i in range(n_samples):
...
def show_batch(x:TensorSequences, y:TensorSequences, samples, ctxs=None, max_n=6, **kwargs):
    ...
    if n_samples > 3:
        ...
        plot_seqs_single_figure(n_samples,n_targ,samples,plot_sequence)
    else:
        #if there are less then 3 samples to plot then put each in its own figure
        plot_seqs_multi_figures(n_samples,n_targ,samples,plot_sequence)
    return ctxs
...
def show_results(x:TensorSequences, y:TensorSequences, samples, outs, ctxs=None, max_n=2, **kwargs):
    ...
    if n_samples > 3:
        #if there are more then 3 samples to plot then put them in a single figure
        plot_seqs_single_figure(n_samples,n_targ,samples,plot_sequence,outs)
    else:
        #if there are less then 3 samples to plot then put each in its own figure
        plot_seqs_multi_figures(n_samples,n_targ,samples,plot_sequence,outs)
    return ctxs