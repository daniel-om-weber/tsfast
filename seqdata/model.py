...
__all__ = ['SeqLinear', 'RNN', 'SimpleRNN', 'AWD_RNN', 'SimpleAWDRNN', 'SimpleGRU', 'SimpleLSTM', 'SimpleQRNN',
           'CausalConv1d', 'CConv1D', 'TCN', 'ConvGRU', 'ConvQRNN', 'Normalizer1D', 'AR_Model', 'AR_GRU']
...
class AWD_RNN(nn.Module):
    "AWD-RNN inspired by https://arxiv.org/abs/1708.02182"
    ...
    def forward(self, inp, h_init=None):
        ...
        return output, torch.stack(new_hid, 0)
...
#Cell
class SimpleAWDRNN(nn.Module):

    @delegates(AWD_RNN, keep=True)
    def __init__(self,input_size,output_size,num_layers=1,hidden_size=100,**kwargs):
        super().__init__()
        self.rnn = AWD_RNN(input_size=input_size,hidden_size=hidden_size,num_layers=num_layers,**kwargs)
        self.final = nn.Conv1d(hidden_size,output_size,kernel_size=1)
