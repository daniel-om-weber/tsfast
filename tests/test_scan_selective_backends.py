...
@pytest.mark.parametrize("mod, name, dev", _BACKENDS)
@pytest.mark.parametrize("shape", SHAPES)
@pytest.mark.parametrize("with_x0", [False, True])
def test_forward_and_grads_match_doubling(mod, name, dev, shape, with_x0):
    B, L, N = shape
    torch.manual_seed(0)
...
@pytest.mark.parametrize("mod, name, dev", _BACKENDS)
def test_deepmamba_param_grads_match_doubling(mod, name, dev):
    ...
    def run(scan_backend):
        # DeepMamba always runs the parallel "scan" path; the process backend preference
        # picks the fused kernel ("auto" -> this backend on its device) or the
        # pure-PyTorch reference.
        with use_backend(scan_backend):
