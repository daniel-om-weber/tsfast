...
# ──────────────────────────────────────────────────────────────────────────────
#  Dual-path equivalence: old pipeline vs new pipeline
# ──────────────────────────────────────────────────────────────────────────────


class TestDualPathEquivalence:
    """Verify old and new pipelines produce equivalent results."""

    def test_wh_window_counts(self):
        """Same number of windows for WH simulation."""
        from tsfast.tsdata.blocks import HDF5Signals
        from tsfast.tsdata.dataset import FileEntry, WindowedDataset
        from tsfast.tsdata.split import get_hdf_files

        train_files = get_hdf_files(WH_PATH / "train")
        valid_files = get_hdf_files(WH_PATH / "valid")

        block = HDF5Signals(["u"])
        train_entries = [FileEntry(path=str(f)) for f in train_files]
        valid_entries = [FileEntry(path=str(f)) for f in valid_files]

        train_ds = WindowedDataset(train_entries, block, HDF5Signals(["y"]), win_sz=100, stp_sz=100)
        valid_ds = WindowedDataset(valid_entries, block, HDF5Signals(["y"]), win_sz=100, stp_sz=100)

        # WH train: 1 file * 80000 samples → (80000-100)//100+1 = 800
        assert len(train_ds) == 800
        # WH valid: 1 file * 20000 samples → (20000-100)//100+1 = 200
        assert len(valid_ds) == 200

    def test_wh_exact_norm_stats_match_old(self):
        """Exact norm stats from HDF5 files should match old pipeline."""
        from tsfast.tsdata.norm import compute_stats_from_files
        from tsfast.tsdata.split import get_hdf_files

        train_files = get_hdf_files(WH_PATH / "train")
        new_u = compute_stats_from_files(train_files, ["u"])
        new_y = compute_stats_from_files(train_files, ["y"])

        # Import old pipeline
        from tsfast.datasets.core import extract_norm_from_hdffiles

        old_u = extract_norm_from_hdffiles(train_files, ["u"])
        old_y = extract_norm_from_hdffiles(train_files, ["y"])

        np.testing.assert_allclose(new_u.mean, old_u.mean, rtol=1e-5)
        np.testing.assert_allclose(new_u.std, old_u.std, rtol=1e-5)
        np.testing.assert_allclose(new_u.min, old_u.min, rtol=1e-5)
        np.testing.assert_allclose(new_u.max, old_u.max, rtol=1e-5)
        np.testing.assert_allclose(new_y.mean, old_y.mean, rtol=1e-5)
        np.testing.assert_allclose(new_y.std, old_y.std, rtol=1e-5)

    def test_batch_shapes_match_old(self):
        """Batch shapes should match between old and new pipeline."""
        from tsfast.tsdata import create_dls as new_create_dls
        from tsfast.datasets.core import create_dls as old_create_dls

        old_dls = old_create_dls(u=["u"], y=["y"], dataset=WH_PATH, win_sz=100, stp_sz=100, num_workers=0, n_batches_train=2)
        new_dls = new_create_dls(u=["u"], y=["y"], dataset=WH_PATH, win_sz=100, stp_sz=100, num_workers=0, n_batches_train=2)

        old_batch = old_dls.one_batch()
        new_batch = new_dls.one_batch()
        assert list(old_batch[0].shape) == list(new_batch[0].shape)
        assert list(old_batch[1].shape) == list(new_batch[1].shape)
