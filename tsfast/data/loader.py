        ...
        if self.sub_seq_len is not None:
            ...
            if self.seq_len % self.sub_seq_len != 0:
                warnings.warn(
                    ...
                    f"The last segment of each sequence in TbpttDl will be shorter.",
                    UserWarning,
                )
        ...
        for b in batch_iter:
            for i in range(self.n_sub_seq):
                ...
                if self.sub_seq_len is None:
                    ...
                    trunc_b = tuple(
                        [retain_type(x[:, i * self.sub_seq_len : (i + 1) * self.sub_seq_len], x) for x in b]
                    )
                yield (
                    trunc_b,
                    (None if torch.utils.data.get_worker_info() is None else torch.utils.data.get_worker_info().id),
                )
...
def WeightedDL_Factory(cls: type) -> type:
    ...
    class WeightedDL(cls):
        ...
        def __init__(self, dataset, wgts=None, **kwargs):
            ...
            if wgts is None:
                if isinstance(self.items, pd.DataFrame) and len(self.items) > 0 and "p_sample" in self.items:
                    self.wgts = self.items.p_sample.to_numpy()
                    self.wgts = self.wgts / self.wgts.sum()
                elif (
                    isinstance(self.items, Iterable)
                    and len(self.items) > 0
                    and hasattr(self.items[0], "keys")
                    and "p_sample" in self.items[0].keys()
                ):
...
def uniform_p_of_float_with_gaps(var_name: str, bins: int = 100):
    ...
    def _inner(df):
        ...
        df_targ["p_sample"] = (
            df_targ["bins"].apply(lambda x: x.length).astype("f8") / length
        )  # sample_prob by bin width
...
def BatchLimit_Factory(cls: type) -> type:
    ...
    class BatchLimitDL(cls):
        ...
        def shuffle_fn(self, idxs):
            return self.rng.sample(idxs, min(len(self) * self.bs, len(idxs)))
