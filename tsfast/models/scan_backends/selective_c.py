...
def _source() -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            # The N axis is tiled into contiguous TILE-wide blocks: one task per
            # (batch, tile) keeps the block's state resident and streams over L, so each
            # step reads/writes one contiguous [n0, n0+nw) run (cache-friendly, and the
            # inner n-loop auto-vectorizes). Parallelism still spans batch x N lanes.
            ...
            "    const int64_t nt = (N + TILE - 1) / TILE;",
            ...
            "    batch_parallel(B * nt, [&](int64_t i0, int64_t i1) {",
            "    for (int64_t i = i0; i < i1; ++i) {",
            "        const int64_t b = i / nt, n0 = (i % nt) * TILE;",
            "        const int nw = (int)std::min<int64_t>(TILE, N - n0);",
            "        float x[TILE];",
            "        for (int j = 0; j < nw; ++j) x[j] = has_x0 ? x0p[b * N + n0 + j] : 0.0f;",
            "        for (int64_t t = 0; t < L; ++t) {",
            "            const int64_t p = (b * L + t) * N + n0;",
            "            const float* lr = lamp + p;",
            "            const float* vr = vp + p;",
            "            float* orr = outp + p;",
            "            for (int j = 0; j < nw; ++j) { x[j] = lr[j] * x[j] + vr[j]; orr[j] = x[j]; }",
            "        }",
            "    }",
            "    });",
            "}",
            ...
            "            float* glr = glamp + p;",
            "            if (t + 1 < L) {",
            "                const float* lnext = lamp + p + N;",
            "                for (int j = 0; j < nw; ++j) G[j] = gr[j] + lnext[j] * G[j];",
            "            } else {",
            "                for (int j = 0; j < nw; ++j) G[j] = gr[j];",
            "            }",
            "            for (int j = 0; j < nw; ++j) gvr[j] = G[j];",
            "            if (t > 0) {",
            "                const float* op = outp + p - N;",
            "                for (int j = 0; j < nw; ++j) glr[j] = G[j] * op[j];",
            "            } else {",
            "                for (int j = 0; j < nw; ++j)",
            "                    glr[j] = G[j] * (has_x0 ? x0p[b * N + n0 + j] : 0.0f);",
            "                if (need_x0) for (int j = 0; j < nw; ++j) gx0p[b * N + n0 + j] = lr[j] * G[j];",
            "            }",
            "        }",
            "    }",
            "    });",
