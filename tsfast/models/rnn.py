...
class RNN(nn.Module):
    ...
    def __init__(self, input_size,hidden_size, num_layers, 
        ...
        self.rnns = nn.ModuleList([self._one_rnn(input_size if i == 0 else hidden_size,
                                                 hidden_size,weight_p,rnn_type,**kwargs) for i in range(num_layers)])
        ...
        if normalization == '':
            ...
            self.norm_layers = nn.ModuleList([nn.LayerNorm(hidden_size,elementwise_affine=False) for _ in range(num_layers)])
